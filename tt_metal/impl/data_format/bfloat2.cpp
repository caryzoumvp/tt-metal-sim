...
#if defined(__AVX2__)
__attribute__((target("avx2"))) static std::vector<float> unpack_bfp2_tiles_into_float_vec_avx2(
...
namespace {
static inline uint32_t bfp2_to_fp32_bits(uint8_t bfp2, uint32_t exp, bool is_exp_a) {
    uint32_t sign = (bfp2 >> 1) & 0x1u;
    uint32_t man = bfp2 & 0x1u;
    if (man == 0) {
        return sign << 31;
    }
    int32_t rebias = is_exp_a ? -112 : 0;
    int32_t exp_adj = static_cast<int32_t>(exp) - rebias;
    return (sign << 31) | (static_cast<uint32_t>(exp_adj) << 23);
}

static inline float bfp2_to_float(uint8_t bfp2, uint32_t exp, bool is_exp_a) {
    uint32_t bits = bfp2_to_fp32_bits(bfp2, exp, is_exp_a);
    float f;
...
static std::vector<float> unpack_bfp2_tiles_into_float_vec_scalar(
    tt::stl::Span<const uint32_t> bfp_tiles,
    bool row_major_output,
    bool is_exp_a,
    const std::optional<tt::tt_metal::Tile>& tile) {
            ...
            for (int tc = 0; tc < subtiles_in_tile_col; ++tc) {
                for (int i = 0; i < subtile_rows; ++i) {
                    ...
                    for (int j = 0; j < subtile_cols; j += num_elements_in_dword) {
                        ...
                        for (int k = 0; k < num_elements_in_dword; ++k) {
                            uint8_t bfp2 = static_cast<uint8_t>((val >> (k * 2)) & 0x3u);
                            float_vec[float_data_index + k] = bfp2_to_float(bfp2, exp, is_exp_a);
                        }
                    }
                }
            }
...
std::vector<float> unpack_bfp2_tiles_into_float_vec(
    ...
    if (__builtin_cpu_supports("avx2")) {
        return unpack_bfp2_tiles_into_float_vec_avx2(bfp_tiles, row_major_output, is_exp_a, tile);
    }
#endif
#endif
#endif
    return unpack_bfp2_tiles_into_float_vec_scalar(bfp_tiles, row_major_output, is_exp_a, tile);
}
